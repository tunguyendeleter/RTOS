 ...
 */
static uint16 Os_NumOfThread = OS_ZERO;
...
Os_ReturnType Os_KernelStackInit(void (*ThreadTask)(void), uint32 StackSize)
{
    ...
    lucNumberOfThread = Os_NumOfThread;
    ...
    *(lpCurrentTCB->stackBegin - STACK_R4_OFFSET) = 0XAAAAAAAAU;                    /* R4 */
    ...
    /* Clear sleep time to zero */
    lpCurrentTCB->SleepTime = 0U; 

    /* Increase num of threads used */
    lucNumberOfThread++;
    Os_NumOfThread = lucNumberOfThread;
...
#endif
    Os_CurrentTCB = Os_CurrentTCB->nextPtr;
#if (THREAD_SLEEP_SUPPORT == STD_ON)
    while (Os_CurrentTCB->SleepTime > 0)
    {
        Os_CurrentTCB = Os_CurrentTCB->nextPtr;
    }
#endif
...
/******************************************************************************
 * Function name:                   :Os_ThreadSleep
 * Description:                     :This function set SleepTime to Thread and put Thread to sleep
 * Input parameter:                 :Time (Time to put Thread to sleep)
 * Output parameter:                :None
 * return:                          :None
 *****************************************************************************/
void Os_ThreadSleep(uint32 Time)
{
    /* Disable global exceptions */
    asm volatile("CPSID I");
    Os_CurrentTCB->SleepTime = Time;
    /* Enable global exceptions */
    asm volatile("CPSIE I");
    Os_ThreadYield();
}

#if (THREAD_SLEEP_SUPPORT == STD_OFF)
/******************************************************************************
 * Function name:                   :Os_UpdateSleepTime
 * Description:                     :This function is placed in exception handler to update SleepTime
 * Input parameter:                 :None
 * Output parameter:                :None
 * return:                          :None
 *****************************************************************************/
void Os_UpdateSleepTime(void)
{
    for (int idx = 0; idx < Os_NumOfThread; idx++)
    {
#if (DYNAMIC_ALLOCATION == STD_ON)
        if (Os_CurrentTCB->SleepTime > 0)
        {
            Os_CurrentTCB->SleepTime--;
        }
        Os_CurrentTCB = Os_CurrentTCB->nextPtr;
#else /* #if (DYNAMIC_ALLOCATION != STD_ON) */
        if (Os_TCB[idx].SleepTime > 0)
        {
            Os_TCB[idx].SleepTime--;
        }
#endif
    }
}
...
/******************************************************************************
 * Function name:                   :Os_GetNumThread
 * Description:                     :This function return number of thread
 * Input parameter:                 :None
 * Output parameter:                :None
 * return:                          :Os_NumOfThread
 *****************************************************************************/
uint16 Os_GetNumThread(void)
{
    return Os_NumOfThread;
}

/******************************************************************************
 * Function name:                   :Os_GetNumPeriThread
 * Description:                     :This function return number of periodic thread
 * Input parameter:                 :None
 * Output parameter:                :None
 * return:                          :Os_NumOfPeriThread
 *****************************************************************************/
uint16 Os_GetNumPeriThread(void)
{
    return Os_NumOfPeriThread;
}
